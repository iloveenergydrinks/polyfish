"""Test whether the Profile format generation complies with OASIS requirements
Verify:
...
def test_profile_formats ():
    ...
    with tempfile .TemporaryDirectory ()as temp_dir :
        ...
        if missing :
            print (f"\n [Error] Required field is missing:{missing }")
        else :
            print (f"\n [PASS] All required fields are present")

        present_optional =set (optional_reddit_fields )&set (reddit_data [0 ].keys ())
        print (f"[Information] Optional fields:{present_optional }")
...
def show_expected_formats ():
    ...
    print ("\n"+"="*60 )
    print ("Profile format reference expected by OASIS")
    print ("="*60 )
