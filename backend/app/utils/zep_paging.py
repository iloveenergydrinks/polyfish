...
Zep's node and edge list APIs use UUID cursor pagination.
This module wraps automatic pagination, including per-page retries,
and transparently returns complete result lists to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page with exponential backoff. Retries only transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes by page, returning up to `max_items` items. Each page request includes retries."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges by page and return the full list. Each page request includes retries."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
