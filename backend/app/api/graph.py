...
def build_graph ():
    ...
    try :
        ...
        if project .status ==ProjectStatus .CREATED :
            return jsonify ({
            "success":False ,
            "error":"The project has not generated ontology yet, please call /ontology/generate first"
            }),400 

        if project .status ==ProjectStatus .GRAPH_BUILDING and not force :
            return jsonify ({
            "success":False ,
            "error":"The map is under construction, please do not submit again. To force a rebuild, add force: true",
        ...
        if force and project .status in [ProjectStatus .GRAPH_BUILDING ,ProjectStatus .FAILED ,ProjectStatus .GRAPH_COMPLETED ]:
            ...
            # Get configuration
        graph_name =data .get ('graph_name',project .name or 'PolyFish Graph')
        ...
        def build_task ():
            ...
            # Update project status failed
                build_logger .error (f"[{task_id }] Map construction failed:{str (e )}")
        ...
        "project_id":project_id ,
        "task_id":task_id ,
        "message":"The graph construction task has been started, please check the progress through /task/{task_id}"
        }
        })
