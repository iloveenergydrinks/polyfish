...
def get_graph_entities (graph_id :str ):
    ...
    try :
        ...
        logger .info (f"Get the graph entity: graph_id={graph_id }, entity_types={entity_types }, enrich={enrich }")
        ...
        logger .error (f"Failed to get graph entity:{str (e )}")
        return jsonify ({
        "success":False ,
        "error":str (e ),
...
def create_simulation ():
    ...
    try :
        ...
        if not graph_id :
            return jsonify ({
            "success":False ,
            "error":"The project has not yet built the graph, please call /api/graph/build first"
            }),400 
...
def _check_simulation_prepared (simulation_id :str )->tuple :
    ...
    try :
        ...
        # Get file statistics
            ...
            logger .info (f"simulation{simulation_id }Test result: Prepared (status={status }, config_generated={config_generated })")
            return True ,{
            "status":status ,
            ...
            logger .warning (f"simulation{simulation_id }Test result: Not ready (status={status }, config_generated={config_generated })")
            return False ,{
...
def prepare_simulation ():
    ...
    try :
        ...
        if not state :
            return jsonify ({
            "success":False ,
            "error":f"The simulation does not exist:{simulation_id }"
            }),404 
        ...
        if not force_regenerate :
            logger .debug (f"Check the simulation{simulation_id }Are you ready to complete...")
            ...
            if is_prepared :
                ...
                "status":"ready",
                "message":"Preparations have been completed, no need to regenerate",
        ...
        def run_prepare ():
            try :
                ...
                progress =0 ,
                message ="Start preparing the simulation environment..."
                )
                ...
                # Calculate total progress
                    ...
                    stage_names ={
                    "reading":"Read map entities",
                    "generating_profiles":"Generate Agent Persona",
                    "generating_config":"Generate simulation configuration",
                    "copying_scripts":"Prepare simulation script"
                    }
        ...
        "task_id":task_id ,
        "status":"preparing",
        "message":"The preparation task has been started, please check the progress through /api/simulation/prepare/status",
    ...
    try :
        ...
        if simulation_id :
            ...
            if is_prepared :
                ...
                "status":"ready",
                "progress":100 ,
                "message":"Preparatory work has been completed",
                "already_prepared":True ,
                "prepare_info":prepare_info 
                }
                })
...
def _get_report_id_for_simulation (simulation_id :str )->str :
    ...
    try :
        ...
        logger .warning (f"Find simulation{simulation_id }The report fails:{e }")
        return None 
...
def get_simulation_config (simulation_id :str ):
    ...
    try :
        ...
        if not config :
            return jsonify ({
            "success":False ,
            "error":f"The simulation configuration does not exist, please call the /prepare interface first"
            }),404 
...
def download_simulation_config (simulation_id :str ):
    ...
    try :
        ...
        if not os .path .exists (config_path ):
            return jsonify ({
            "success":False ,
            "error":"The configuration file does not exist, please call the /prepare interface first"
            }),404 
    ...
    try :
        ...
        # Check if preparations are complete
            ...
            # Preparation is complete, check if there are any running processes
                ...
                # Check if the simulated process is actually running
                    ...
                    # The process is actually running
                        ...
                        # Force mode: Stop a running simulation
                            logger .info (f"Force mode: Stop a running simulation{simulation_id }")
                            try :
                            ...
                            return jsonify ({
                            "success":False ,
                            "error":f"The simulation is running, please call the /stop interface to stop it first, or use force=true to force a restart."
                            }),400 
                ...
                logger .info (f"simulation{simulation_id }The preparation work has been completed and the reset status is ready (original status:{state .status .value }）")
            ...
            # Preparation work not completed
                return jsonify ({
                "success":False ,
                "error":f"Simulation is not ready, current status:{state .status .value }, please call /prepare interface first"
                }),400 
        ...
        # Get graph_id from simulation state or project
            ...
            if not graph_id :
                return jsonify ({
                "success":False ,
                "error":"Enabling graph memory updates requires a valid graph_id, please ensure that the project has built the graph"
                }),400 
    ...
    try :
        ...
        for i ,interview in enumerate (interviews ):
            if 'agent_id'not in interview :
                return jsonify ({
                "success":False ,
                "error":f"Interview List No.{i +1 }Item is missing agent_id"
                }),400 
            if 'prompt'not in interview :
                return jsonify ({
                "success":False ,
                "error":f"Interview List No.{i +1 }Item is missing prompt"
                }),400 
            ...
            if item_platform and item_platform not in ("twitter","reddit"):
                return jsonify ({
                "success":False ,
                "error":f"Interview List No.{i +1 }The platform of the item can only be 'twitter' or 'reddit'"
                }),400 
