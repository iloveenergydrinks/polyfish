...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Get the entity type (exclude the default Entity tag)"""
...
class ZepEntityReader:
    ...
    def filter_defined_entities(
        ...
        # Get all edges (for subsequent association lookups)
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []
        
        # Construct a mapping of node UUID to node data
        ...
        for node in all_nodes:
            ...
            # If a predefined type is specified, checks for a match
        ...
        logger.info(f"Filtering completed: total nodes{total_count}, eligible{len(filtered_entities)}, "
