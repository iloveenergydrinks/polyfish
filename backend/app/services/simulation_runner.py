...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if existing and existing.runner_status in [RunnerStatus.RUNNING, RunnerStatus.STARTING]:
            raise ValueError(f"The simulation is already running:{simulation_id}")
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                try:
                    ...
                    # Forced termination
                    logger.warning(f"The process is not responding and is forced to terminate:{simulation_id}")
                    subprocess.run(
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if state.runner_status not in [RunnerStatus.RUNNING, RunnerStatus.PAUSED]:
            raise ValueError(f"The simulation is not running:{simulation_id}, status={state.runner_status}")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(sim_dir):
            return {"success": True, "message": "The simulation directory does not exist and does not need to be cleaned"}
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    if state:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = "The server is shut down and the simulation is terminated"
                        cls._save_run_state(state)
                    ...
                    try:
                        ...
                        logger.info(f"Try updating state.json:{state_file}")
    ...
    def register_cleanup(cls):
        ...
        # SIGHUP only exists in Unix systems (macOS/Linux), not in Windows
        ...
        try:
            ...
            logger.warning("Unable to register signal handler (not on main thread), only use atexit")
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"The simulation environment is not running or has been closed, and the Interview cannot be executed:{simulation_id}")
    ...
    def close_simulation_env(
        ...
        logger.info(f"Send the shutdown environment command: simulation_id={simulation_id}")
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment shutdown command sent (timeout waiting for response, environment may be shutting down)"
            }
