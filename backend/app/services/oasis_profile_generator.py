...
class OasisProfileGenerator :
    ...
    use_llm :bool =True 
        ...
        Args:
            ...
            use_llm: whether to use LLM to generate detailed personality
    ...
    def _search_zep_for_entity (self ,entity :EntityNode )->Dict [str ,Any ]:
        ...
        comprehensive_query =f"about{entity_name }All information, activities, events, relationships and context of"
            ...
            for attempt in range (max_retries ):
                try :
                    ...
                    if attempt <max_retries -1 :
                        logger .debug (f"Zep edge search{attempt +1 }failed:{str (e )[:80 ]}, retrying...")
                        time .sleep (delay )
                        delay *=2 
                    else :
                        logger .debug (f"Zep edge search in{max_retries }Still failed after attempts:{e }")
            return None 
        ...
        def search_nodes ():
            ...
            for attempt in range (max_retries ):
                try :
                    ...
                    last_exception =e 
                    if attempt <max_retries -1 :
                        logger .debug (f"Zep node search No.{attempt +1 }failed:{str (e )[:80 ]}, retrying...")
                        time .sleep (delay )
                        delay *=2 
                    else :
                        logger .debug (f"Zep node search in{max_retries }Still failed after attempts:{e }")
            return None 
        ...
        # Execute edges and nodes searches in parallel
            ...
            logger .info (f"Zep hybrid search completed:{entity_name }, get{len (results ['facts'])}facts,{len (results ['node_summaries'])}related nodes")
    ...
    def _build_entity_context (self ,entity :EntityNode )->str :
        ...
        # Deduplication: exclude existing facts
            ...
            if new_facts :
                context_parts .append ("### Factual information retrieved by Zep\\n"+"\n".join (f"- {f }"for f in new_facts [:15 ]))
    ...
    def _try_fix_json (self ,content :str ,entity_name :str ,entity_type :str ,entity_summary :str ="")->Dict [str ,Any ]:
        ...
        logger .warning (f"JSON repair failed, returning to infrastructure")
        return {
    ...
    def _get_system_prompt (self ,is_individual :bool )->str :
        ...
        base_prompt ="You are an expert in social media persona generation. Generate detailed and realistic characters for public opinion simulation to restore the existing reality to the greatest extent. Valid JSON format must be returned, all string values ​​cannot contain unescaped newlines. Use Chinese."
        return base_prompt 
    ...
    context :str 
        ...
        return f"""Generate detailed social media user profiles for entities to restore the existing reality to the greatest extent.

Entity name:{entity_name }Entity type:{entity_type }Entity summary:{entity_summary }Entity properties:{attrs_str }Contextual information:{context_str }Please generate JSON with the following fields:

1. bio: social media introduction, 200 words
2. Persona: Detailed personality description (2000 words of plain text), which must include:
...
- Speeches from institutional accounts must conform to their identity and positioning"""
    ...
    output_platform :str ="reddit"
        ...
        def save_profiles_realtime ():
            ...
            # Filter out generated profiles
                ...
                try :
                    ...
                    logger .warning (f"Real-time saving of profiles failed:{e }")
        ...
        logger .info (f"Start parallel build{total }Agent personas (number of parallels:{parallel_count }）...")
        print (f"\n{'='*60 }")
        ...
        print (f"\n{'='*60 }")
        print (f"The character generation is completed! symbiosis{len ([p for p in profiles if p ])}Agent")
        print (f"{'='*60 }\n")
    ...
    def _save_twitter_csv (self ,profiles :List [OasisAgentProfile ],file_path :str ):
        ...
        logger .info (f"saved{len (profiles )}Twitter Profile to{file_path }(OASIS CSV format)")
    ...
    def _save_reddit_json (self ,profiles :List [OasisAgentProfile ],file_path :str ):
        ...
        logger .info (f"saved{len (profiles )}Reddit Profile to{file_path }(JSON format, including user_id field)")
