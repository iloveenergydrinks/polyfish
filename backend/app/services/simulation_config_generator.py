...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        total_steps = 3 + num_batches  # Time configuration + event configuration + N batches of Agents + platform configuration
        current_step = 0
        ...
        report_progress(1, "Build time configuration...")
        ...
        report_progress(2, "Generate event configurations and hot topics...")
        ...
        reasoning_parts.append(f"Agent configuration: successfully generated{len(all_agent_configs)}indivual")
        ...
        reasoning_parts.append(f"Initial post allocation:{assigned_count}posts have assigned publishers")
        ...
        logger.info(f"Simulation configuration generation is completed:{len(params.agent_configs)}Agent configuration")
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        prompt = f"""Based on the following simulation requirements, a time simulation configuration is generated.{context_truncated}## Task
...
- reasoning (string): A brief explanation of why this is configured"""
...
{context_truncated}## Available entity types and examples{type_info}## Task
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(f"Type ' not found{poster_type}' Matching Agent, use the Agent with the highest influence")
                if agent_configs:
...
Simulation requirements:{simulation_requirement}## Entity list
```json{json.dumps(entity_list, ensure_ascii=False, indent=2)}```
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are an expert in analyzing social media behavior. Return pure JSON, and the configuration must conform to Chinese people's work and rest habits."
