...
4. Return JSON format: {"sub_queries": ["Sub question 1", "Sub question 2", ...]}"""
        ...
        user_prompt = f"""Simulation requirements background:{simulation_requirement}

{f"Report context:{report_context[:500]}" if report_context else ""}Please break down the following questions into{max_queries}Sub-question:{query}Returns a list of subquestions in JSON format."""
...
Return JSON format:
{
    ...
    "reasoning": "Explanation of reasons for selection"
        ...
        user_prompt = f"""Interview requirements:{interview_requirement}Simulation background:{simulation_requirement if simulation_requirement else "Not provided"}Selectable Agent list (total{len(agent_summaries)}indivual):{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}Please select the most{max_agents}Which Agent is most suitable for interviewing and explain the reasons for selection."""
...
Return JSON format: {"questions": ["Question 1", "Question 2", ...]}"""
        ...
        user_prompt = f"""Interview requirements:{interview_requirement}Simulation background:{simulation_requirement if simulation_requirement else "Not provided"}Interviewee role:{', '.join(agent_roles)}Please generate 3-5 interview questions."""
...
- Do not use dividing lines (such as ---, ***)
        - Use standard double quotes ("") when quoting an interviewee's original words
- You can use **bold** to mark keywords, but do not use other Markdown syntax"""

        user_prompt = f"""Interview topic:{interview_requirement}Interview content:{"".join(interview_texts)}Please generate an interview summary."""
        ...
        try:
            ...
            return f"Interviewed {len(interviews)} participants in total, including: " + ", ".join([i.agent_name for i in interviews])
