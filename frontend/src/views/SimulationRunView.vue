...
// Read maxRounds from the query during initialization so the child component gets it immediately
...
const handleGoBack = async () => {
  // Shut down the running simulation before returning to Step 2
  addLog('Preparing to return to Step 2 and close the simulation...')
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close the environment, trying a forced stop...`)
        try {
      ...
      // If the environment is not running, check whether the process still needs to stop
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Fetch simulation config so we can read minutes_per_round
      try {
        ...
        addLog(`Failed to load time configuration, using default: ${minutesPerRound.value} min/round`)
      }
...
const loadGraph = async (graphId) => {
  // While simulating, skip fullscreen loading during auto-refresh to avoid flicker
...
onMounted(() => {
  ...
  // Record the maxRounds setting, which is already read from the query
  if (maxRounds.value) {
