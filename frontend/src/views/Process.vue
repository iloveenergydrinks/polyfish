      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph construction will start automatically after ontology generation completes</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Graph BuildTranslated</p>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Translated，LLMTranslated，Translated(Entity Types + relationshipsTranslated)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Use the generated ontology to chunk the document and call the Zep API to build a knowledge graph and extract entities and relationships.
                </div>
              </div>
              
              <!-- translated comment -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Waiting for ontology generationTranslated...</div>
              </div>
...
const ontologyProgress = ref(null) // translated comment
const currentPhase = ref(-1) // translated comment
const selectedItem = ref(null) // translated comment
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Graph BuildTranslated'
  if (currentPhase.value === 0) return 'Ontology GenerationTranslated'
  return 'Initializing'
})
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending uploaded file was found. Return to the home page and try again.'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // translated comment
    ontologyProgress.value = { message: 'Uploading the file and analyzing the document...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Ontology GenerationTranslated'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'Project initialization failed: ' + (err.message || 'UnknownTranslated')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = 'Failed to load project: ' + (err.message || 'UnknownTranslated')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'TranslatedGraph Build...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph BuildTranslated...'
      ...
      error.value = response.error || 'TranslatedGraph BuildTranslated'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'TranslatedGraph BuildTranslated: ' + (err.message || 'UnknownTranslated')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph BuildTranslated，Translated...')
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Build Complete，Translated...'
        }
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 Translated:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph BuildTranslated: ' + (task.error || 'UnknownTranslated')
        buildProgress.value = null
      }
    }
  } catch (err) {
