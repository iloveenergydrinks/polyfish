    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG long-term and short-term memory is updating in real time' : 'Updating in real time...' }}
        </div>
        
        <!-- Hint shown after the simulation ends -->
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing. Refresh the graph manually in a moment.</span>
...
const showSimulationFinishedHint = ref(false) // Hint shown after the simulation ends
const wasSimulating = ref(false) // Track whether the graph was simulating previously

// Dismiss the end-of-simulation hint
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

//Monitor isSimulating changes and detect the end of simulation
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    //Change from simulation to non-simulation state, display end prompt
...
//Computed entity type used in legend
...
const renderGraph = () => {
  ...
  //Process edge data and calculate the number and index of edges between the same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} //Self-loop edges grouped by nodes
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  //Count the number of edges between each pair of nodes, collected from ring edges
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      //Self loop - collected into array
  ...
  //Record which edge of each pair of nodes is currently processed
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() //Processed self-link points
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      //Self-loop edge - only one merged self-loop is added to each node
    ...
    //Determine whether the direction of the edge is consistent with the standardized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    //Calculate curvature: when multiple edges are spread out, a single edge is a straight line
    let curvature = 0
    if (totalCount > 1) {
      //Evenly distributed curvature ensures clear distinction
      //The curvature range increases according to the number of edges. The more edges, the greater the curvature range.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      //If the direction of the edge is opposite to the normalized direction, flip the curvature
      //This ensures that all edges are distributed in the same reference system and will not overlap due to different directions.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      //Dynamically adjust the distance based on the number of edges between the pair of nodes
      //Base distance 150, increase by 40 for each additional side
    ...
    //Add gravity towards the center, allowing independent node groups to gather into the central area
  ...
  //Links - support curves using path
  ...
  const getLinkPath = (d) => {
    ...
    //Detect self-loop
    if (d.isSelfLoop) {
      //Self-loop: draw an arc starting from the node and returning
      const loopRadius = 30
      //Start from the right side of the node and go back in a circle
      const x1 = sx + 8  //Starting point offset
      const y1 = sy - 4
      const x2 = sx + 8  //end offset
      const y2 = sy + 4
      //Use arcs to draw self-loops (sweep-flag=1 clockwise)
    ...
    //Calculate curve control points - dynamically adjust based on number of edges and distances
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    //The offset perpendicular to the direction of the connection is calculated based on the distance ratio to ensure that the curve is clearly visible.
    //The more edges there are, the greater the offset is as a proportion of the distance.
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 //Basic 25%, increase by 5% for each additional side
  ...
  const getLinkMidpoint = (d) => {
    ...
    //The midpoint of the quadratic Bezier curve is t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
  ...
  //Save references for external control to show and hide
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        //Only record the position and do not restart the simulation (distinguish between click and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        //Detect if drag actually starts (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          //The simulation is restarted after real drag is detected for the first time.
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        //Only after actual dragging will the simulation gradually stop.
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    //Update edge label position (no rotation, clearer horizontal display)
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') //Remove rotation, keep level
    })
...
//Monitor edge label display switch
