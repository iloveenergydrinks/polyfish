    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Generate the result report' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- CREATE_COMMENT: publish a comment -->
                ...
                <!-- Generic fallback for unknown types or unhandled content -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Maximum round count passed from Step 2
  minutesPerRound: {
    type: Number,
    default: 30 // Default to 30 minutes per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const actionIds = ref(new Set()) // Action IDs used for deduplication
const scrollContainer = ref(null)

// Computed
// Display actions in chronological order with the newest at the bottom
...
// Format elapsed simulation time from round count and minutes per round
...
// Elapsed simulation time for the Twitter platform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Elapsed simulation time for the Reddit platform
...
// Reset all state before restarting the simulation
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Stop any polling left from the previous run
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state so the previous run cannot leak into the next one
  resetAllState()
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Force a clean restart
      enable_graph_memory_update: true  // Enable dynamic graph updates
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Setting the maximum simulation rounds to ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared old simulation logs and restarted cleanly')
      }
...
onMounted(() => {
  addLog('Step 3 simulation runtime initialized')
  if (props.simulationId) {
    doStartSimulation()
  }
})
...
.platform-status.twitter .platform-icon { color: #888888; }
.platform-status.reddit .platform-icon { color: #888888; }
...
.breakdown-divider { color: #444444; }
.breakdown-item.twitter { color: #888888; }
.breakdown-item.reddit { color: #888888; }
...
.timeline-item.twitter .marker-dot { background: #888888; }
.timeline-item.reddit .marker-dot { background: #888888; }
.timeline-item.twitter .timeline-marker { border-color: #888888; }
.timeline-item.reddit .timeline-marker { border-color: #888888; }
...
.badge-post { background: #1a1a1a; color: #d4d4d4; border-color: #333333; }
.badge-comment { background: #1a1a1a; color: #888888; border-color: #333333; }
.badge-action { background: #111111; color: #888888; border: 1px solid #333333; }
.badge-meta { background: #0a0a0a; color: #666666; border: 1px dashed #444444; }
...
@keyframes ripple {
  0% { transform: scale(0.8); opacity: 1; border-color: #444444; }
  100% { transform: scale(2.5); opacity: 0; border-color: #333333; }
}
...
.log-content::-webkit-scrollbar-thumb { background: #333; border-radius: 0; }
...
.log-time { color: #888888; min-width: 75px; }
.log-msg { color: #666666; word-break: break-all; }
